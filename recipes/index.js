...
function RatingMarkup(rating) {
	...
	return `<span class="rating" role="img" aria-label="Rating: ${safeRating} out of 5 stars">${stars}</span>`;
}

function RecipeCard(recipe) {
	const firstTag = recipe.tags?.[0] ?? 'recipe';
	const description = recipe.description ?? '';
	return `
		<article class="recipe">
			<img src="${recipe.image}" alt="${recipe.name}" />
			<div class="recipe-info">
				<p class="tag">${firstTag}</p>
				<h2>${recipe.name}</h2>
				<p class="description">${description}</p>
				${RatingMarkup(recipe.rating ?? 0)}
			</div>
		</article>
	`;
}
...
function getRecipes(list) {
	...
	main.innerHTML = list.map(RecipeCard).join('');
}
